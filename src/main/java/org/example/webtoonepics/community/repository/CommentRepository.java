...
public interface CommentRepository extends JpaRepository<C_comment, Long> {
    ...
    @Query("select new org.example.webtoonepics.community.dto.C_commentDto(c.id, c.content,c.user.userName, c.created_at) from C_comment c inner join c.community u where u.id = :id")
