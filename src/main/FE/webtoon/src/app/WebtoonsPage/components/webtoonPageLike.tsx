...
export default function WebtoonsPage() {
  ...
  const [page, setPage] = useState<number>(0); // 현재 페이지 상태 (0부터 시작)
    ...
    const fetchWebtoons = async () => {
      ...
      try {
        const response = await axios.get(
          `${process.env.NEXT_PUBLIC_API_URL}/likes?page=${page}` // 전체 웹툰 데이터 API 호출
        );
  ...
  // 페이지 네이션 버튼에서 보여줄 페이지 번호는 사용자에게 1부터 보이도록 그대로 두되, API에서는 0부터 처리
  ...
  const handleNextGroup = () => {
    if ((pageGroup + 1) * buttonsPerGroup < totalPages) {
      setPage((pageGroup + 1) * buttonsPerGroup); // 페이지 설정 수정
    }
  };
